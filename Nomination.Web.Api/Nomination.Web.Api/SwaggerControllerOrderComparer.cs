...
//https://github.com/domaindrivendev/Swashbuckle/issues/1262
namespace Nomination.Web.Api
{
    ...
    public class SwaggerControllerOrderComparer<T> : IComparer<string>
    {
        ...
        public int Compare(string controllerX, string controllerY)
        {
            ...
            if (!_orders.TryGetValue(controllerX, out xOrder))
                xOrder = int.MaxValue;
            if (!_orders.TryGetValue(controllerY, out yOrder))
                yOrder = int.MaxValue;

            // If sortorder values differ, return the result
            if (xOrder != yOrder)
                return xOrder.CompareTo(yOrder);
            // If sortoder values are equal, we fall back to ordering by name
